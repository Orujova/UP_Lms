...
const MultiAttachmentUploadEdit = ({
  attachments = [],
  onChange,
  onMarkForDeletion,
}) => {
    ...
    <div className="space-y-4">
          ...
          <div className="w-full">
            <div className="grid grid-cols-1 gap-3">
              {attachments.map((attachment, index) => (
                <div
                  key={index}
                  className={`relative group flex items-center p-3 ${
                    markedForDeletion.includes(attachment.id)
                ...
                >
                  ...
                  {attachment.isExisting ? (
                    <button
                      type="button"
                      onClick={() => toggleMarkForDeletion(attachment.id)}
      ...
      {markedForDeletion.length > 0 && (
        <div className="text-xs text-red-500">
          ...
          {markedForDeletion.length === 1 ? "attachment" : "attachments"} marked
          for deletion
        </div>
      )}
...
export default MultiAttachmentUploadEdit;
