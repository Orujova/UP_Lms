...
const EditVacancyPage = () => {
  ...
  // Fetch required data for dropdowns and current vacancy
  useEffect(() => {
    if (id) {
  ...
  // Populate form with current vacancy data when available
  useEffect(() => {
    if (currentVacancy && (currentVacancy.vacancyId || currentVacancy.id)) {
      const salaryRange = currentVacancy.salaryRange || "";
      let minSalary = "";
      let maxSalary = "";

      if (salaryRange !== "Negotiable") {
        const salaryParts = salaryRange.split("-").map((part) => part.trim());
        if (salaryParts.length === 2) {
          minSalary = salaryParts[0];
          maxSalary = salaryParts[1];
        }
      }

      const submissionDate = currentVacancy.lastSubmissionDate
        ? new Date(currentVacancy.lastSubmissionDate)
            .toISOString()
            .split("T")[0]
        : "";

      setFormData({
        id: currentVacancy.vacancyId || currentVacancy.id,
  ...
  // Update selected target groups when targetGroups is loaded and targetGroupIds are available
  useEffect(() => {
    if (
      targetGroups &&
  ...
  if (
    loading &&
    (!departments.length || !lineManagers.length || !currentVacancy)
  ) {
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Update the details of the existing vacancy
              </p>
            </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-700 mt-1">
                    ...
                    changes will immediately take effect once saved.
                  </p>
                </div>
              </div>
            </div>
