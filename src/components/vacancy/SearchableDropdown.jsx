...
const SearchableDropdown = ({
  ...
  // Get selected option display value - with safe access
  ...
  // Filter options based on search value - with safety checks
  const filteredOptions = safeOptions.filter((option) => {
    // Skip if option is null/undefined or displayKey doesn't exist
    if (!option || typeof option[displayKey] === "undefined") return false;

    // Convert to string first to ensure toLowerCase exists
    const optionDisplay = String(option[displayKey] || "");
    const search = String(searchValue || "").toLowerCase();

    return optionDisplay.toLowerCase().includes(search);
  });
      ...
      <div className="relative">
        <div
          ...
          className={`flex items-center justify-between w-full px-3 py-2 text-xs border ${
            ...
            isOpen ? "border ring-0 border-[#01DBC8]" : ""
          }`}
        >
